...
def spotify(id, type):
    ...
    r = requests.post(token_url, token_data)
    ...
    if type == 'artist':
        artist_id = id
        r = requests.get(base_url + 'artists/' + artist_id, headers=token_headers)
        r = r.json()

        return r['name']
    else: 
        album_id = id
        r = requests.get(base_url + 'albums/' + album_id, headers=token_headers)
        r = r.json()
